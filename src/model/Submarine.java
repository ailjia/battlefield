package model;
import java.awt.Color;
import java.util.List;

public class Submarine extends Ship {

	// this class inherits from the ship and specifies a particular type of ship

	private final int pointPerHit = 3;  // unique point after hitting the battleship
	private final static int shipLength = 3; // the length for this kind of ship
	private final static Color color = Color.RED; // the color for this kind of ship

	public Submarine(List<Coordinates> cList) {
		// this constructs the ship with a list of coordinates

		super(cList);
		name = "submarine ship";
	}


	public Submarine() {

		// sets up the ship randomly with the particular length
		super(setup(shipLength));
		name = "submarine ship";

	}

	public int getScoreMultipiler() {
		// when the ship is sinking, the score will double with respect to the original score per hit
		return super.getScoreMultipiler() * this.pointPerHit;
	}

	public int getShiplength() {

		// get the length of the ship
		return shipLength;
	}

	public int getPointperhit() {
		// get the point per hit for this kind of ship
		return pointPerHit;
	}


	@Override
	public int getType() {
		// get the type of the ship, each ship type represents by a unique integer
		return 3;
	}

	@Override
	public Color getColor() {
		// get the color of the ship
		return color;
	}

}
